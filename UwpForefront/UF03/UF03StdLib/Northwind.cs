  ...
  public class Northwind
  {
    ...
    private const string CONN_STR_SQLSERVER
      = @"Data Source=Win10VM-RS3.corp.BluewaterSoft.jp\SQLEXPRESS;"
        ...
        + "User ID=nwtestuser;Password=;Persist Security Info=False;";
    // ※ Android / iOS では DNS による名前解決ができる必要がある。
    //    Windows は、NetBIOS による名前解決でも OK のようだ。
    //    名前解決できないときは、ホスト指定を IP アドレスにする（例☟）。
    //= @"Data Source=192.168.0.12\SQLEXPRESS;Initial Catalog=NORTHWIND;User ID=nwtestuser;Password=;Persist Security Info=False;";
    ...
    // ※ Windows 認証 (Active Directory によるユーザー認証) を使う場合、
    ...
    // ※ UWP アプリでは、マニフェストで以下のケイパビリティの付与が必要
    //    SQL ユーザー認証の場合: プライベートネットワーク
    //    Windows 認証の場合: エンタープライズ認証
